...
function fillemq() {
    ...
    box = theForm.elements['active0roster'];
    iar = box.length;
...
function rostermove(old1, new1) {
    ...
    document.forms["packages"].spb2a0.disabled = true;
    document.forms["packages"].spb2d0.disabled = true;
    ...
    if ((new1 == "inactive0roster") || (new1 == "inactive1roster"))
        //if (theForm.elements[new1].length >= 5) {
            //theForm.elements['warncut'].value = 'OK';
            //ax1 = "No more than 5 players per team can be offered in an individual trade. "
            //alert(ax1);
            //icango = false;
        //}


    if ((ph1 < 50) && (new1 == 'cutroster')) {
        icango = false;
        ax1 = "You are not allowed to cut players that are physically unable to perform. "
        ax1 = ax1 + "You must either wait for the player to heal, put him on the inactive list, or send him to the injured reserve."
        alert(ax1);
    }
        ...
        if (old2 > -1) {
            if ((old1 != 'activeroster') || (theForm.elements[old1].length > 40)) {
                if ((new1 != 'activeroster') || (theForm.elements[new1].length < 46)) {
                    ...
                    new1a = 'Active0'
                    //alert(new1);
                    if (new1 == 'inactive0roster') {
                        new1a = 'Inactive0';
                    }
                    if (new1 == 'active1roster') {
                        new1a = 'Active1';
                    }
                    if (new1 == 'inactive1roster') {
                        new1a = 'Inactive1';
                    }
    ...
    document.forms["packages"].spb2a0.disabled = false;
    document.forms["packages"].spb2d0.disabled = false;
}
...
function okaccept() {
    ...
    if (theForm.elements['acc1'].value == theForm.elements['accb'].value) {
        document.forms["packages"].spba.disabled = true;
        document.forms["packages"].spbb.disabled = true;
        x[0].innerHTML = "<B>Accepting this trade.. please wait!</b>";
        x[0].bgColor = "#99ee99";
        //alert(theForm.elements['acc1'].value);
        document.forms["packages"].submit();
        goon = false;
    }
    theForm.elements['acc1'].value = theForm.elements['accb'].value;
    if (goon) {
        alert("You are choosing to ACCEPT this trade; click the accept button once more to confirm.  Once this has been done, IT CANNOT BE REVERSED!");
    }
}
...
function okreject() {
    ...
    var goon = true;
    var x = document.getElementById('storeme').rows[2].cells;
    if (theForm.elements['acc1'].value == theForm.elements['acca'].value) {
        document.forms["packages"].spba.disabled = true;
        document.forms["packages"].spbb.disabled = true;
        x[0].innerHTML = "<B>Rejecting this trade.. please wait!</b>";
        x[0].bgColor = "#99ee99";
        //alert(theForm.elements['acc1'].value);
        document.forms["packages"].submit();
        goon = false;
    }
    theForm.elements['acc1'].value = theForm.elements['acca'].value;
    if (goon) {
        alert("You are choosing to REJECT this trade; click the reject button once more to confirm.  Once this has been done, IT CANNOT BE REVERSED!");
    }
}
...
function okweregood() {
    ...
    document.forms["packages"].spb.disabled = true;
    var x = document.getElementById('storeme').rows[2].cells;
    x[0].innerHTML = "<B>Offering this trade..  Please wait!</b>";
    x[0].bgColor = "#99ee99";
    // alert(2);
    document.forms["packages"].submit();
...
function storethese() {
    ...
    if (theForm.elements['inactive1roster'].length == 0) {
        //theForm.elements['warncut'].value = 'OK';
        ax1 = "You have not offered any of your players to trade! "
        alert(ax1);
        okgo = false;
    }

    if ((theForm.elements['inactive0roster'].length == 0) && okgo) {
        ax1 = "You have not asked for any players in return for this trade!"
        alert(ax1);
        okgo = false;
    }
    ...
    if (okgo) {
        ...
        var x = document.getElementById('storeme').rows[2].cells;
        document.forms["packages"].spb.disabled = true;
        x[0].innerHTML = "<B>Moving to the confirmation screen..  Please wait!</b>";
        x[0].bgColor = "#99ee99";
        pid = theForm.elements['pid'].value;
        pexper = theForm.elements['pexper'].value.split("!");
        pitb = theForm.elements['pitb'].value.split("!");
        pitbt = theForm.elements['pitbt'].value.split("!");

        for (loop = 1; loop <= 2; loop++) {
            onpit = '1';
            lookat = 'inactive1roster';
            lookata1 = 'Y';
            if (loop == 2) {
                onpit = '0';
                lookat = 'inactive0roster';
                lookata1 = 'Y';
            }
            //alert(pid)
            if (theForm.elements[lookat].length > 0) {
                ...
                for (x = 0; x < box.length; x++) {
                    ...
                    lookatb = pitb[v1];
                    lookatbt = pitbt[v1];
                    lloo = lloo + trim(x1) + "^" + trim(lookata) + "^" + pexper[v1] + "^" + pitbt[v1] + "^!"
                }
            }
        }
        //alert(lloo)
...
function updateplayer(z, z1, mpos, ozsent) {
    ...
    if (parseFloat(z1a) > 0) {
        ...
        x2 = "<table width=\"100\%\" bgcolor=#666666 cellpadding=1 cellspacing=1>"
        loopon0 = 0;
        ...
        x1 = "<table width=\"100%\" bgcolor=#666666 cellpadding=1 cellspacing=0>"
        backstart = ""
        ...
        ozsenta = "My Team"
        xcolor = "#99dd99"
        if ((ozsent == "Inactive0") || (ozsent == "Active0")) {
            xcolor = "#dd9999"
            ozsenta = document.getElementById("otname").value;
        }

        x1 = x1 + "<tr><th align=left bgcolor=\"" + xcolor + "\">Plays For : " + ozsenta + "</th></tr>"
        ...
        if ((document.getElementById('thisisme').value == "True")) {
            x1 = x1 + "<tr><th width=150 align=left>&nbsp;</th><td>"
...
function updateplayersalary(myplayer, fn, ln, posi) {
    ...
    for (x = 8; x >= 2; x--) {
        ...
        x1[3].innerHTML = "";
        var theForm = document.forms["packages"];
        if (theForm.elements['rosterswap'].value == "") {
            x1[4].innerHTML = "";
        }
        if (x < 9) {
            x1[0].innerHTML = "";
